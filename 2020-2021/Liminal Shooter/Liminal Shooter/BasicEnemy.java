/**
 * [BasicEnemy.java]
 * Represents an enemy that only shoots one bullet at a time
 * @author Edison Du
...
class BasicEnemy extends Enemy {
   ...
   */
  BasicEnemy (int x, int y, int width, int height, int health, int speed, int range, int movementRange, int fireRate, int bulletSpeed) {
    super(x, y, width, height, health, speed, range, movementRange, fireRate, bulletSpeed);
    this.setGun(new BasicGun(x-5, x-5, 60, 60, fireRate, bulletSpeed));
  }
  ...
  public static void loadSprite() {
    sprite = loadSprite("Sprites/enemy-1.png");
  }
