/**
 * [Shotgun.java]
 * Represents a gun that shoots many bullets in a cone
 * @author Edison Du
...
class Shotgun extends Gun {
   ...
   */
  Shotgun (int x, int y, int width, int height, int fireRate, int bulletSpeed) {
    super(x, y, width, height, fireRate, bulletSpeed);
  }
  
  /**
   * shoot
   * Creates 6 projectiles with angles in a cone shape and adds them to an ArrayList of projectiles
  ...
  public boolean shoot(ArrayList<Projectile> projectileList) {
    if (this.getFireCooldown() <= 0) {
      ...
      for (double i = -0.15; i <= 0.15; i += 0.05) {
        projectileList.add(new Projectile(this.getCenterX(),this.getCenterY(),20,20,angle+i,this.getBulletSpeed(),10));
      }
  ...
  public static void loadSprites() {
    for (int i = 0; i < 9; i++) {
      sprites[i] = loadSprite("Sprites/gun_shotgun-" + i + ".png");
    }
  }
