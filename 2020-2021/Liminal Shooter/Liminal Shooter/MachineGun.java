/**
 * [MachineGun.java]
 * Represents a gun that shoots rapidly but less accurately
 * @author Edison Du
...
class MachineGun extends Gun {
   ...
   */
  MachineGun (int x, int y, int width, int height, int fireRate, int bulletSpeed) {
    super(x, y, width, height, fireRate, bulletSpeed);
  }
  
  /**
   * shoot
   * Creates a projectile with a slightly offset angle and adds it to an ArrayList of projectiles
  ...
  public boolean shoot(ArrayList<Projectile> projectileList) {
    if (this.getFireCooldown() <= 0) {
      ...
      offsetAngle = Math.atan2(this.getTargetY()-this.getCenterY()-10, this.getTargetX()-this.getCenterX()-10);
      offsetAngle += (Math.random()/3) * (random.nextInt(2) == 1 ? 1 : -1);
      projectileList.add(new Projectile(this.getCenterX(),this.getCenterY(),20,20,offsetAngle,this.getBulletSpeed(),7));
  ...
  public static void loadSprites() {
    for (int i = 0; i < 9; i++) {
      sprites[i] = loadSprite("Sprites/gun_machine-" + i + ".png");
    }
  }
